package day03;

import java.lang.reflect.Array;
import java.util.Arrays;
import java.util.Scanner;

public class StudentManagePark {

	public static void main(String[] args) {

		Scanner scan = new Scanner(System.in);

		//학생정보를 등록할 배열을 while문 바깥에 선언.
		String[] nameList = new String[100];
		int[] ageList = new int[100];
		String[] infoList = new String[100];

		//고객수
		int count = 0;
		//조회할 위치
		int index = -1; //


		x:while(true) {
//			System.out.println(Arrays.toString(nameList));
//			System.out.println(Arrays.toString(ageList));
//			System.out.println(Arrays.toString(infoList));
			
			System.out.println("[정보]고객수:" + count + ", 조회 위치:" + index);
			System.out.println("[메뉴]1.추가, 2. 이전정보, 3.다음정보, 4.현재정보, 5.정보수정, 6.정보삭제, 7.종료");
			System.out.println("================================================================");
			System.out.println("메뉴입력:");
			String menu = scan.next();

			switch (menu) {
			case "1":
				/*
				 * 이름,성별,간단한 정보를 입력 받아서 각각의 배열에 순서대로 저장될 수 있게 처리합니다.
				 * 
				 * 입력 후에는 count를 증가해야한다.
				 * 
				 */
				System.out.println("==============사용자 정보입력================");
				System.out.print("이름 : ");
				String name = scan.next();
				nameList[count] = name;
				System.out.print("나이 : ");
				int age = scan.nextInt();
				ageList[count] = age;
				System.out.print("정보 : ");
				String info = scan.next();
				infoList[count] = info;
				count ++;
				index ++;
				System.out.println("================================================================");
				break;
			case "2":
				/*
				 * 이전정보출력:
				 * 정보출력은 index위치로 출력합니다.
				 * 이전정보출력을 누르면 index를 -1 시키고 해당 위치의 정보를 출력
				 * 조건
				 * index가 0보다 작다면, 출력X 처리.
				 */
				if(index>0) {
					index --;
					System.out.println("이름 : "+nameList[index]);
					System.out.println("나이 : "+ageList[index]);
					System.out.println("정보 : "+infoList[index]);
					System.out.println("================================================================");
				}else {
					System.out.println("이전 정보가 없습니다.");
					System.out.println("================================================================");
				}
				break;
			case "3":
				/*
				 * 다음정보출력:
				 * 다음출력은 index 위치로 출력한다.
				 * 다음정보출력을 누르면 index를 +1시키고 해당위치에 정보를 출력.
				 * 조건
				 * index가 count-1 보다 같거나크면, 출력X 처리. // index는 0부터 count는 1부터 시작해서 1차이.
				 */
				if(index<count-1) {
					index ++;
					System.out.println("이름 : "+nameList[index]);
					System.out.println("나이 : "+ageList[index]);
					System.out.println("정보 : "+infoList[index]);
					System.out.println("================================================================");
				}else {
					System.out.println("다음 정보가 없습니다.");
					System.out.println("================================================================");
				}
				break;
			case "4":
				/*
				 * 현재정보출력
				 * index가 가르키고 있는 위치정보를 출력.
				 * 조건
				 * 출력할 수 있는 조건을 생각해서 처리. index와 count 사이에 있을 것이다.
				 */
				if(index>=0 && index <=count-1) {
					System.out.println("이름 : "+nameList[index]);
					System.out.println("나이 : "+ageList[index]);
					System.out.println("정보 : "+infoList[index]);
					System.out.println("================================================================");
				}else {
					System.out.println("현재 위치에 정보가 없습니다.");
					System.out.println("================================================================");
				}
				break;
			case "5":
				/*
				 * 정보수정
				 * 새로운 이름,나이,정보를 입력받아서
				 * 현재 위치를 수정해주면 됨.
				 * 조건 
				 * 4번과 동일
				 */
				if(index>=0 && index <=count-1) {
					System.out.println("=========현재 정보 수정==========");
					System.out.println(index + "번째 위치를 수정합니다.");
					System.out.print("이름 : ");
					String newName = scan.next();
					nameList[index] = newName;
					System.out.print("나이 : ");
					int newAge = scan.nextInt();
					ageList[index] = newAge;
					System.out.print("정보 : ");
					String newInfo = scan.next();
					infoList[index] = newInfo;
					System.out.println("================================================================");
				}
				break;
			case "6":
				/*
				 * 현재정보 삭제.
				 * 현재 삭제하려는 index부터 ~~~뒤에 있는 배열요소를 당겨와 덮어 씌움.
				 * 사람수도 감소시켜준다.
				 * 조건
				 * :4번과 동일
				 */
				System.out.println(nameList[index]+ "==========정보 삭제===========");
				System.out.println(nameList[index]+"님 정보가 삭제되었습니다.");
				if(index>=0 && index <=count-1) {
					nameList[index] = nameList[index+1];
					ageList[index] = ageList[index+1];
					infoList[index] = infoList[index+1];
					count --;
					index --;
					System.out.println("================================================================");
				}
				break;
			case "7":
				/*
				 * while문을 탈출
				 * 
				 */
				System.out.println("종료합니다.");
				System.out.println("================================================================");
				break x;
			default:
				System.out.println("잘못 입력했습니다.");
				System.out.println("================================================================");
				break;
			}
		}

	}
}
