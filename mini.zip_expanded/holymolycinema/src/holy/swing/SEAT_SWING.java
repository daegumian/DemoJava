...
public class SEAT_SWING extends JFrame {
   ...
   public SEAT_SWING(HmemberVO hv ,ScheduleVO sv ) {
	   this.hv = hv;
	   this.sv = sv;
	   System.out.println(this.hv.getMemId());
	   initialize();
   }
   ...
   public void initialize() {
	   ...
	   String currentDirectory = System.getProperty("user.dir");
		String testpath = currentDirectory + "\\img\\seat.jpg";
	  MainPanel = new ImagePanel(new ImageIcon(testpath).getImage());
...
//            seat = btnNewButton.getText();
      ...
      btnNewButton_3.setBounds(50, 130, 110, 29);
      ...
      JButton btnNewButton_15 = new JButton("선택 완료");
      btnNewButton_15.setForeground(Color.DARK_GRAY);
      btnNewButton_15.setBackground(Color.WHITE);
      btnNewButton_15.setFont(new Font("티머니 둥근바람 Regular", Font.PLAIN, 15));
      btnNewButton_15.setBounds(170, 130, 114, 29);
      ...
      JLabel lblPaymentMethod = new JLabel("결제방법  :");
      lblPaymentMethod.setFont(new Font("티머니 둥근바람 Regular", Font.PLAIN, 15));
      lblPaymentMethod.setBounds(295, 130, 114, 29);
      MainPanel.add(lblPaymentMethod);

      String[] paymentMethods = {"신용카드", "실시간 계좌이체", "홀리몰리시네마 예매권"};
      JComboBox<String> comboBoxPayment = new JComboBox<>(paymentMethods);
      comboBoxPayment.setBounds(370, 130, 170, 29);
      MainPanel.add(comboBoxPayment);
      
      JButton btnNewButton_14 = new JButton("결제 하기");
      ...
      btnNewButton_14.setBounds(550, 130, 114, 29);
      ...
      btnNewButton_14.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				 ...
				 // 결제수단 변수
				String payment = comboBoxPayment.getSelectedItem().toString();
				...
				int price = film.getPrice(sv.getFilmtitle());
				TicketingVO tv = new TicketingVO("00",now,seatlist.size(),(price * seatlist.size()),payment,sv.getScheNum());
				
				TicketDAO td = new TicketDAO();
				td.insertTicket(tv);
			    
				/*
			    TicketListVO tlv = new TicketListVO();
			    tlv.setTickno(td.getTickno());
			    tlv.setMemid(hv.getMemId());
			    //tlv.setTheanum(sv.getTheaNum());
			    seatlist
			    tlv.setSeatnum(sv.se)
			    td.insertTicketList(tlv);
			    */
			    for (int i = 0; i < seatlist.size(); i++) {
			    	TicketListVO tlv = new TicketListVO();
			    	tlv.setTickno(td.getTickno());
			    	tlv.setMemid(hv.getMemId());
			    	tlv.setSeatnum(seatlist.get(i));
			    	td.insertTicketList(tlv);
				}
