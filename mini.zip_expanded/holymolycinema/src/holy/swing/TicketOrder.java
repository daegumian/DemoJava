...
public class TicketOrder {
	...
	public TicketOrder(HmemberVO v) {
		initialize();
		this.v2 = v;
		System.out.println(v.getMemPhone());
	}
	...
	public void initialize() {
		...
		String testpath = currentDirectory + "\\img\\ticketorder.jpg";
		...
		list.addListSelectionListener(new ListSelectionListener() {
			...
			public void valueChanged(ListSelectionEvent e) {
				...
				filmtitle = (String) list.getSelectedValue();
				ArrayList<String> aa = new ArrayList<>();
				List<ScheduleVO> sb = sv.getDetail(filmtitle);
				String detail = sb.get(0).getFilmContent();
				textPane.setText(detail);
				
				StringBuilder sb2 = new StringBuilder(filmtitle);
				String answer="";
		...
		list_1.addListSelectionListener(new ListSelectionListener() {
			@Override
			public void valueChanged(ListSelectionEvent e) {
				date2 = (String) list_1.getSelectedValue();
				
				ArrayList<String> aa = new ArrayList<>();
				List<ScheduleVO> sb = sv.getDetail(filmtitle,date2);
				for(ScheduleVO a : sb) {
		...
		list_3.addListSelectionListener(new ListSelectionListener() {
			@Override
			public void valueChanged(ListSelectionEvent e) {
				time = (String) list_3.getSelectedValue();				
			}
		});
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				ScheduleVO sv2=sv.getDetail(filmtitle, date2, time); // 스케줄객체
				SEAT_SWING seat = new SEAT_SWING(v2,sv2);
		...
		//sv2 = sv.getDetail(filmtitle, date2, time);
	}
}
